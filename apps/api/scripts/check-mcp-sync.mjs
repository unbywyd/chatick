...
for (const file of ['index.ts']) {
  ...
  //
  // Флаг s (dotAll) обязателен: без него точка не совпадает с переносом
  // строки, и вызов, разбитый форматтером на несколько строк, переставал
  // находиться. Путь молча выпадал из проверки — та же беда, что описана
  // выше, только новым способом: счёт проверенных упал с 42 до 41, и это
  // единственное, что её выдало.
  const re = /call(?:<[^>]*>)?\(.*?,\s*'(GET|POST|PATCH|DELETE)',\s*(`[^`]+`|'[^']+')/gs

...
describe('только личный диалог с ассистентом', () => {
  it('картинки берутся в ai-ветке, а не в общем чате', () => {
    ...
    // Якорь — на ветке ОТПРАВКИ, с открывающей скобкой: голое «mode === 'ai'»
    // встречается и в подсчёте непрочитанного выше, и проверка читала не тот
    // кусок файла.
    const at = route.indexOf("if (mode === 'ai') {")
    expect(at, 'ветка отправки в ai не найдена').toBeGreaterThan(-1)
    const ai = route.slice(at)

...
/**
 * Разметка, присланная в неготовом виде.
 *
 * Все три случая — не выдуманные: они лежали в базе знаний живыми записями.
 * Пять заметок ушли в базу с текстом вида `&lt;p dir=&quot;rtl&quot;&gt;`
 * вместо абзацев — человек открывал редактор и видел сырые теги.
 *
 * Общая цена ошибки здесь необратима: markdown экранирует теги и кладёт
 * результат в базу навсегда. Отличить их потом от текста, который человек
 * написал сам, уже нельзя.
 */
describe('неготовая разметка чинится до сохранения', () => {
  it('экранированный HTML разворачивается обратно в разметку', () => {
    // Ровно то, что легло в базу: ассистент «показал» разметку вместо того,
    // чтобы её применить. looksLikeHtml не видел ни одного настоящего тега,
    // текст уходил в markdown — и теги становились видимым текстом.
    //
    // Саботаж: убрать разэкранирование — вернётся `<p>&lt;p ...` в базе.
    const html = richText('&lt;p dir="rtl"&gt;&lt;strong&gt;הסיבה&lt;/strong&gt; — x&lt;/p&gt;')
    expect(html).toContain('<strong>הסיבה</strong>')
    expect(html, 'экранированный тег доехал до базы').not.toContain('&lt;p')
  })

  it('одиночная «меньше» в тексте про код разметкой не считается', () => {
    // Обратная сторона: разэкранируя по одной `&lt;`, мы бы испортили
    // нормальную запись. Поэтому ищем ПАРУ — открывающий и закрывающий тег.
    //
    // Саботаж: искать одиночную `&lt;` — эта запись сломается.
    const html = richText('<p>if a &lt; b then stop</p>')
    expect(html, 'сравнение превратилось в тег').toContain('&lt;')
  })

  it('HTML в заборе ```html освобождается от забора', () => {
    // Ассистента просят HTML — он присылает его так, как показывают код.
    // Обратные кавычки доезжали до записи и были в ней видны.
    const html = richText('```html\n<p>text</p>\n```')
    expect(html).toContain('<p dir="ltr">text</p>')
    expect(html, 'забор остался в тексте').not.toContain('```')
  })

  it('безымянный забор остаётся кодом', () => {
    // Забор без языка означает противоположное: «это код, покажи как есть».
    // Сняв его, мы превращали адрес в ссылку, а команду — в абзац.
    //
    // Саботаж: сделать язык необязательным (```(?:html?)?) — команда станет
    // ссылкой, и тест про домен в блоке кода упадёт вместе с этим.
    const html = richText('```\ncurl example.com\n```')
    expect(html, 'команда превратилась в ссылку').not.toContain('<a ')
  })

  it('markdown рядом с настоящим HTML не теряется', () => {
    // Смешанный вход: короткое замыкание на HTML оставляло «## Head»
    // видимой решёткой. marked живые теги не трогает, поэтому смешанное
    // безопаснее гнать через разбор.
    //
    // Саботаж: вернуть `looksLikeHtml(s) ? s : ...` — решётка станет текстом.
    const html = richText('## Head\n\n<p dir="rtl">shalom</p>')
    expect(html).toContain('<h2')
    expect(html, 'заголовок остался решёткой').not.toContain('## Head')
    expect(html, 'живой тег не сохранился').toContain('<p dir="rtl">shalom</p>')
  })

  it('список после готового абзаца дорисовывается, а не остаётся дефисами', () => {
    // Самый частый смешанный вход: ассистент закрыл абзац тегом, а
    // перечисление под ним написал markdown-ом. Без hasMarkdown строки
    // оставались бы дефисами в тексте.
    const html = richText('<p>one</p>\n\n- first\n- second')
    expect(html).toContain('<li')
    expect(html, 'дефисы остались текстом').not.toMatch(/^- first/m)
  })

  it('HTML принимается по-прежнему, хотя инструкция просит markdown', () => {
    // Инструменты и гайд теперь говорят «пиши markdown»: рукописный HTML —
    // единственное, что однажды приехало экранированным, а в markdown
    // экранировать нечего. Но ПРИНИМАТЬ HTML мы обязаны и дальше: его шлёт
    // веб-редактор (Tiptap), и он лежит во всех уже сохранённых записях,
    // которые проходят через правку.
    //
    // Саботаж: начать отвергать HTML — правка любой существующей заметки из
    // веба разложит разметку в текст.
    const html = richText('<p dir="rtl"><strong>x</strong></p>')
    expect(html).toBe('<p dir="rtl"><strong>x</strong></p>')
  })

  it('нумерованный список и заголовок после готового абзаца тоже дорисовываются', () => {
    // Те же ветки hasMarkdown, что и дефис: цифра с точкой и решётка.
    // Проверяем их отдельно — вырезав любую из них, мы бы вернули в базу
    // видимые «1.» и «#», и один тест про дефисы этого не заметил бы.
    const ol = richText('<p>a</p>\n\n1. one\n2. two')
    expect(ol, 'нумерация осталась текстом').toContain('<li')

    const head = richText('<p>a</p>\n\n# Head')
    expect(head, 'решётка осталась текстом').toContain('<h1')
  })
})

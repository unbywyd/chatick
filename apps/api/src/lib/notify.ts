    ...
    for (const user of recipients) {
      ...
      const [created] = await db
        ...
        .values({
          ...
          projectId,
          /**
           * Компания обязательна, хотя колонка её не требует.
           *
           * Инбокс отбирает по company_id (routes/inbox.ts): без него
           * уведомление не попадает в ленту НИКОГДА — при этом исправно
           * считается в счётчике сайдбара. Человек видит «1», открывает
           * проект и не находит ничего: показывать нечего, а цифра стоит.
           *
           * Так и случилось: 103 уведомления родились без компании, потому
           * что колонку добавили миграцией 0093 и заполнили только прошлые, а
           * здесь проставить забыли. Проект уже загружен строкой выше —
           * компанию берём из него.
           */

...
describe('разбор натикавшего', () => {
  ...
  it('часы — только третьим числом', () => {
    ...
    // секунды. Другого способа ввести часы нет, и догадываться не о чем.
    expect(parseElapsed('2:00')).toBe(2)
    expect(parseElapsed('2:00:00')).toBe(120)
    expect(parseElapsed('20:10:00')).toBe(1210)
  })
...
describe('правка не ломает дату', () => {
  it('новое начало считается от округлённого «сейчас»', () => {
    ...
    expect(desktop).toContain('base.getTime() - minutes * 60_000')
  })

  it('секунды обнуляются у точки отсчёта, а не у результата', () => {
    // setSeconds ПОСЛЕ вычитания сдвигал начало ещё на текущие секунды:
    // вводишь 10:10, а счётчик показывает 10:10:47.
    expect(desktop).toContain('base.setSeconds(0, 0)')
    expect(desktop).not.toContain('startedAt.setSeconds')
  })
...
describe('ввод не даёт напортачить', () => {
  ...
  it('панель узнаёт о правке сразу', () => {
    ...
    const at = desktop.indexOf('const base = new Date()')
    const body = desktop.slice(at, at + 900)
    expect(body).toMatch(/'desktop-running'/)
  })
})

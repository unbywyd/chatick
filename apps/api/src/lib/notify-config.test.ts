...
describe('срок и номер задачи на иврите', () => {
  ...
  it('номер задачи отделён от названия', () => {
    // Иначе номер и название слипаются: «testTASK-2».
    //
    // Раньше здесь проверялся unicode-bidi:isolate, но замер в браузере
    // показал, что инлайном этого не добиться вовсе: в RTL-строке первый
    // элемент И ЕСТЬ самый правый, и никакое bidi-свойство или поле этого не
    // меняет. Порядок задаётся разворотом flex-строки — см. task-row-rtl.
    //
    // Проверяем следствие, а не приём: номер читается слева направо, а
    // название остаётся самостоятельным куском при любом языке.

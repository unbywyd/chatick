...
export async function aiChatReply(projectId: string, userId: string, userMessage: string): Promise<string | null> {
  ...
  return completeWithTools(cfg, {
    system: [
      ...
      'Tools: read_chat (the group conversation), conversation summaries, full-history search, files (list_files, attach_file_to_task), task CRUD (create/update_task can set assignee, due date, estimate, priority, status and sprint), sprints (list_sprints), task comments (add_task_comment writes ON BEHALF OF the user; list_task_comments reads them), and resources (list_resources, create_resource).',
      'When assigning tasks, use the person the TEAM section says is responsible for that area. Assign by name; use list_sprints to find valid sprint names before setting one.',

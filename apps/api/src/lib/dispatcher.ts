...
export async function aiChatReply(
  ...
  return completeWithTools(cfg, {
    system: [
      ...
      // Язык — двумя отдельными правилами, потому что вопросов тут два, и
      // раньше был ответ только на один. Из «только ответ может быть на языке
      // пользователя» модель не узнаёт, какой язык считать пользовательским, и
      // берёт его из окружения: имена в ростере команды русские — значит
      // отвечает по-русски человеку, написавшему на иврите.
      `LANGUAGE OF YOUR REPLY: answer in the SAME language the person just wrote to you in. They wrote in Hebrew — reply in Hebrew; in Russian — reply in Russian. Never switch because names, task titles or earlier messages in the context are in another language. If their message is too short to tell, use the project language (${lang}).`,
      ...
      'DOCUMENTS: SEARCH BEFORE READING. read_document with query="…" returns only the matching passages and their offsets — a 30k-character spec is 8 sequential reads otherwise, and you usually need one paragraph. list_documents query="…" searches every document at once when you do not know which one covers it. Without a query read_document returns a CHUNK: it tells you the total length and the next offset, so call it again with a bigger offset. To add text use append_to_document (never resend a whole long document).',

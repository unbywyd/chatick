...
const envSchema = z.object({
  ...
  /**
   * Ключ для эмбеддингов — поиск по смыслу.
   *
   * Отдельный от ключа компании намеренно: у Anthropic эмбеддингов нет вовсе,
   * а StartPlan работает именно на нём. Ключ один на сервер, платит владелец
   * площадки — при нынешних объёмах это доли цента.
   *
   * Не задан — поиск по смыслу просто выключен, обычный продолжает работать.
   */

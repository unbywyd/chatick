...
describe('сверка MCP с мостом не слепнет на переносах', () => {
  // Проверка ищет вызовы call(...) регуляркой. Без флага s точка не совпадает
  // с переносом строки — и вызов, разбитый форматтером на несколько строк,
  // переставал находиться. Путь молча выпадал из проверки.
  //
  // Так и случилось: счёт проверенных упал с 42 до 41 после одной правки, а
  // когда флаг вернули — поднялся до 48. То есть невидимыми были СЕМЬ
  // вызовов, не один.
  //
  // Выдало это только число в выводе. Сама проверка рапортовала об успехе —
  // ровно то, от чего она должна защищать.
  it('регулярка читает многострочные вызовы', () => {
    const script = readFileSync(join(import.meta.dirname, '../../scripts/check-mcp-sync.mjs'), 'utf8')
    const line = script.split('\n').find((l) => l.includes('const re = /call'))
    expect(line, 'регулярка вызовов не найдена').toBeTruthy()
    expect(line!.trimEnd().endsWith('/gs'), 'потерян флаг s — многострочные вызовы снова невидимы').toBe(true)
  })
})

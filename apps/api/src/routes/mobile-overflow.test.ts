...
describe('поля ввода не держат ширину на телефоне', () => {
  ...
  it('поиск людей тянется по месту', () => {
    // Требование то же: поле занимает свободное место, а не держит
    // фиксированные 160px. Форма изменилась с w-full на flex-1, когда рядом
    // встал переключатель периода: w-full ТРЕБУЕТ целую строку и утаскивал
    // переключатель на вторую, а в иврите вся группа отрывалась от заголовка
    // и уезжала к левому краю. flex-1 растягивает так же, но делит строку.
    //
    // Саботаж: вернуть w-40 без flex-1 — поле снова станет нерастяжимым.
    expect(people).toMatch(/className="h-8 min-w-0 flex-1 max-w-56 text-sm"/)
  })
})
...
describe('сетки перестраиваются в одну колонку', () => {
  it('карточки проектов и людей — по одной на телефоне', () => {
    ...
    // У метрик колонок на десктопе стало четыре — важно не их число, а то,
    // что многоколоночность начинается НЕ РАНЬШЕ sm. Проверяем правило, а не
    // цифру: иначе тест падает на каждой добавленной карточке, хотя вёрстка
    // на телефоне не менялась.
    const metrics = /className="grid gap-3 ([^"]*)"/.exec(overview)?.[1] ?? ''
    expect(metrics, 'ряд метрик исчез').toBeTruthy()
    expect(metrics, 'метрики в несколько колонок уже на телефоне').not.toMatch(/(?:^| )grid-cols-[0-9]/)
    expect(metrics, 'метрики не перестраиваются вовсе').toMatch(/sm:grid-cols-[0-9]/)
  })
})
...
describe('плейсхолдер чата помещается в панель', () => {
  it('короткий, но клавишу называет', () => {
    // Было «Написать в группу… Ctrl+Enter — отправить»: в узкой панели не
    // помещалось и уезжало в прокрутку. Убрать подсказку целиком нельзя —
    // голый Enter переносит строку, и человек не понимает, почему сообщение
    // не уходит (см. send-hint.test.ts). Оставили самое короткое, что ещё
    // отвечает на вопрос.
    //
    // Саботаж: вернуть длинную формулировку — прокрутка вернётся.
    for (const lang of ['ru', 'en', 'he']) {
      ...
      const p = json.chat.placeholderGroup
      expect(p, `${lang}: клавиша пропала из плейсхолдера`).toMatch(/\{\{key\}\}\+Enter/)
      // Предел на длину: столько влезает в узкую панель. Прежнее «Написать в
      // группу… {{key}}+Enter — отправить» — 41 знак, и оно не помещалось.
      expect(p.length, `${lang}: плейсхолдер снова длинный (${p})`).toBeLessThanOrEqual(30)
    }
  })
})

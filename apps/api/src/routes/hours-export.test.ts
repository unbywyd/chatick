...
describe('файл настоящий, а не CSV', () => {
  it('пишется через xlsx', () => {
    ...
    // Проверяем ПРАВИЛО (пишем через XLSX, расширение .xlsx), а не точный вид
    // имени: в нём появилась ветка для «за всё время», где дат нет вовсе, и
    // дословный шаблон падал бы на любой правке имени файла.
    expect(tab, 'файл пишется не через xlsx').toContain('XLSX.writeFile(wb,')
    expect(tab, 'расширение не .xlsx').toContain('.xlsx`')
  })
...
describe('период', () => {
  it('«всё время» доступно, и «этот год» не потерялся', () => {
    // РАНЬШЕ здесь стояло обратное: «всё время» прятали, опасаясь поднять
    // каждую запись компании — «при дюжине человек за пять лет тысяч тридцать».
    //
    // Опасение не подтвердилось, и ошибка была в рассуждении: свод считает
    // СУБД (group by человек × проект), наружу едет строка на пару, а не на
    // запись, и объём ответа от длины периода почти не зависит. Замер на
    // проде: весь период по компании — 0.19 мс при 186 записях.
    //
    // Без него нельзя ответить на «сколько всего занял проект» — вопрос, ради
    // которого отчёт и открывают, когда выставляют счёт. Календарь не выручает:
    // даты начала проекта человек не помнит.
    ...
    expect(list, '«всё время» снова спрятано').toMatch(/'all'/)

...
describe('«за всё время» переживает круг через адрес', () => {
  const overview = readFileSync(
    join(import.meta.dirname, 'companies.ts'),
    'utf8',
  ).replace(CRLF, LF)
  ...
  it('выбор помечается в адресе отдельным ключом', () => {
    // «За всё время» — это ПУСТЫЕ даты, а пустое значение выпадает из строки
    // запроса. Без отдельного ключа выбор не переживал круг «записали →
    // прочитали»: пустота неотличима от «ничего не выбрано», и подставлялся
    // текущий месяц. Человек выбирал «всё время», получал сентябрь.
    //
    // Саботаж: писать только from/to — тест падает.
    expect(tab, 'выбор не помечается в адресе').toContain("params.get('period') === 'all'")
    expect(tab, 'пустой период не помечается при записи').toContain("patchParams({ period: 'all'")
  })

  it('обычный период снимает пометку', () => {
    // Иначе «всё время» залипнет: ключ остался бы в адресе поверх новых дат.
    expect(tab, 'пометка не снимается при обычном периоде').toContain('patchParams({ period: null')
  })

  it('обзор компании не выдаёт текущий месяц за «всё время»', () => {
    // Умолчание там — текущий месяц, и без отдельного ключа экран показал бы
    // сентябрь под подписью «за всё время». Числа выглядят правдоподобно и
    // молча врут — это хуже, чем отсутствие самой опции.
    //
    // Саботаж: убрать ветку allTime на сервере — тест падает.
    expect(overview, 'сервер обзора не знает про «всё время»').toContain("q.period === 'all'")
    expect(overviewTab, 'клиент обзора не шлёт пометку').toContain('overview?period=all')
  })
})

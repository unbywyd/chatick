...
describe('привязка ресурсов к задаче', () => {
  const body = handler('patch', '/tasks/:id')

  it('привязка ресурсов считается изменением', () => {
    // Поле было в списке допустимых, но ручка считала тело пустым и отвечала
    // «Nothing to update»: разрешено, задокументировано и не работает —
    // худший вид поддержки.
    expect(body).toMatch(/b\.resourceIds === undefined/)
  })

  it('запрос без полей задачи не падает на пустом обновлении', () => {
    // set() с пустым объектом — ошибка драйвера: привязка ресурса не трогает
    // полей самой задачи.
    expect(body).toMatch(/Object\.keys\(patch\)\.length/)
  })
})

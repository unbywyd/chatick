...
export async function notifyTask(
  ...
  /**
   * «Готово» НЕ уведомляет — это единственный статус, о котором не сообщаем.
   *
   * Остальные переходы значат «нужно твоё участие»: задачу взяли, задача ждёт
   * проверки. «Готово» значит обратное — участие больше не нужно, и делать с
   * этим нечего.
   *
   * Замер на живых данных: из 304 уведомлений о статусе 111 приходились на
   * done, и 70 из них никто не открыл. У Даниэля мимо прошли 95%, у Хадиль
   * 71%. Остальные статусы читают: непрочитанных почти нет.
   *
   * О закрытии и так узнают: в чат уходит автосообщение «сделал TASK-18»
   * (postTaskDone ниже), и оно приходит раньше, чем человек откроет инбокс.
   */
  const isDone = task.status === 'done'
  if (opts.statusChanged && !isDone && statusRecipients.length) {
    await notify({
      projectId,
      event: 'task_status',

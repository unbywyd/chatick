...
describe('сайдбар', () => {
  ...
  it('кнопка меню видна всегда, а не по наведению', () => {
    // Спрятанное под hover находит только тот, кто уже знает, что оно там
    // есть — о самой возможности скрыть проект человек так и не узнает. Плюс
    // на телефоне наведения нет вовсе.
    //
    // Саботаж: вернуть opacity-0 + group-hover — функция снова станет
    // невидимой.
    const at = sidebar.indexOf('sidebar.projectActions')
    const btn = sidebar.slice(at - 300, at + 900)
    expect(btn, 'кнопка снова прячется до наведения').not.toMatch(/opacity-0/)
    // И мишень не меньше 32px: в неё надо попасть, а не целиться.
    expect(btn).toMatch(/size-8/)
  })

  it('под кнопку освобождено место', () => {
    // Кнопка лежит поверх строки: без правого отступа название и время
    // уезжают под неё.
    expect(sidebar).toMatch(/ps-2 pe-10 text-start/)
  })

...
describe('интерфейс', () => {
  ...
  it('из архива видно, как выйти', () => {
    /**
     * Кнопка меняла подпись, но значок оставался тот же — «архив». Человек уже
     * в архиве, а на кнопке снова архив: выход не читался, и было непонятно,
     * что это вообще та же кнопка.
     */
    const start = app('screens/StartScreen.tsx')
    expect(start, 'значок не меняется вместе с действием').toMatch(
      /showArchived \? <ArrowLeft[\s\S]{0,80}?: <Archive/,
    )
  })

  it('пустой архив не врёт, что проектов нет', () => {
    // «Проектов пока нет» человек читал бы сразу после того, как их видел.
    const start = app('screens/StartScreen.tsx')
    expect(start).toMatch(/showArchived \? 'start\.archiveEmpty' : 'start\.noProjects'/)
  })

...
companiesRoute.get('/:companyId/overview', async (c) => {
  ...
  const now = new Date()
  /**
   * period=all — «за всё время»: нижнюю границу опускаем.
   *
   * Отличать его от «клиент не прислал период» обязательно. Пустые даты
   * выпадают из строки запроса, и без отдельного ключа выбор «за всё время»
   * приезжал бы сюда неотличимым от «ничего не выбрано» — а умолчание здесь
   * текущий месяц. Экран показал бы сентябрь под подписью «за всё время»:
   * числа выглядят правдоподобно и молча врут.
   *
   * Заведомо ранняя дата, а не null: границы ниже подставляются в SQL как
   * параметры, и ветвление на каждый запрос стоило бы дороже. Записей
   * раньше 2000 года у трекера быть не может.
   */
  const allTime = q.period === 'all'
  const since = allTime

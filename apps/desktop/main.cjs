...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      spellcheck: true,
      // Окно живёт свёрнутым часами, но именно оно держит сокет и собирает
      // состояние для панели в трее. С обычным поведением Electron таймеры и
      // отрисовка в скрытом окне душатся, и уведомление доезжало до трея
      // только когда человек разворачивал приложение — то есть ровно тогда,
  ...
  return Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    {
      ...
      click: async () => {
        ...
        // Панель — отдельное окно, и раньше «Обновить» её не касалось. Пока
        // она была вшита, обновлять было нечего; теперь она грузится с сайта
        // и может залипнуть на старой версии ровно так же. Закрываем и
        // выбрасываем: при следующем открытии перечитается заново.
        if (panel) {
          panel.destroy()
          panel = null
        }

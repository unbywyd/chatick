...
export function LoginScreen() {
  ...
  async function requestCode() {
    ...
    try {
       ...
       * 429 — НЕ ошибка входа.
       *
       * Он означает, что письмо ушло меньше минуты назад и код уже лежит в
       * почте. Для человека это ровно тот же успех: код у него на руках,
       * вводить его можно прямо сейчас.
       *
       * Показывали красное «Не удалось войти» и не открывали поле ввода —
       * получалось, что код есть, а деть его некуда, и экран уверяет, что всё
       * сломано. Повторное нажатие упиралось в тот же ответ.
       *
       * Поэтому ведём себя как при успехе: открываем поле и говорим спокойно.
       */
      if (e instanceof ApiError && e.status === 429) {
        setOtpSent(true)
        toast.success(t('login.otpAlreadySent'))
      } else {

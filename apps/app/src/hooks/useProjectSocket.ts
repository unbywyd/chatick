...
export type SocketEvents = {
  ...
  /** Ассистент открыл что-то в рабочей зоне: путь внутри проекта. */
  onOpenInUi?: (p: { path: string }) => void
    ...
    const connect = () => {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (event === 'open_in_ui') eventsRef.current.onOpenInUi?.(payload as { path: string })

...
const PRESETS: PresetKey[] = [
  ...
  'last90',
  'thisYear',
  // «Всё время» вернулось. Убирали его из опасения поднять каждую запись
  // компании — «за пять лет тысяч тридцать, секунды ожидания».
  //
  // Опасение не подтвердилось, и ошибка была в рассуждении, а не в числах:
  // свод считает СУБД (group by человек × проект), наружу едет строка на пару,
  // а не строка на запись. Объём ответа от длины периода почти не зависит.
  // Замер на проде: весь период по компании — 0.19 мс, запросов 186.
  //
  // А без него нельзя ответить на «сколько всего занял проект» — вопрос, ради
  // которого отчёт и открывают, когда выставляют счёт. Календарь тут не
  // помогает: дату начала проекта человек не помнит.
  'all',
]

 ...
 *
 * Просили «дату создания в таблице», но вопрос за просьбой был другой:
 * «заходишь и пытаешься понять, где старые таски, а где новые». Дата на него
 * не отвечает — двадцать дат надо сравнивать в уме; «17д» отвечает сразу.
 *
 * Отдельной колонки нет намеренно: их девять при min-w-640px, таблица и без
 * того тесная. Место нашлось под номером — колонка узкая и наполовину пустая.
 *
 * Сегодняшние молчат: «0д» — не ответ, а шум в каждой новой строке. Со
 * вчерашней и дальше показываем число.
 */
function TaskAge({ createdAt, status }: { createdAt: string; status: string }) {
  ...
  return (
    <div
      // Точная дата — в подсказке: метка отвечает «сколько уже», а когда
      // именно, спрашивают редко и уже прицельно.

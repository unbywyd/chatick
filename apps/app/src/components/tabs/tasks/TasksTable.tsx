  ...
  return (
    ...
    >
      ...
      {/* Номер не переносим: «TASK-8» рвался на две строки, и вся строка
          ...
          Под номером — возраст задачи. Отдельной колонки «Создана» нет
          намеренно: колонок девять, таблица и без того тесная, а дата в ней
          не отвечает на вопрос, ради которого её просили — «где старые, а где
          новые». Двадцать дат приходится сравнивать в уме; «17д» отвечает
          сразу. Место нашлось под номером: колонка узкая и наполовину
          пустая. */}
...
/**
 * Возраст задачи мелко под номером.
 *
 * Просили «дату создания в таблице», но вопрос за просьбой был другой: «где
 * старые, а где новые». Дата на него не отвечает — двадцать дат приходится
 * сравнивать в уме. «17д» отвечает сразу.
 *
 * СВЕЖИЕ НЕ ПОМЕЧАЕМ ВОВСЕ. Если подписать каждую строку, метка перестанет
 * что-либо выделять: колонка «17д, 3д, 1д, 6д» — это те же двадцать чисел для
 * сравнения. Молчание у свежих и есть ответ «эта новая», а глаз цепляется
 * ровно за долгожители.
 *
 * Порог — три недели, и он выбран ЗАМЕРОМ, а не на глаз. На живых данных
 * (289 открытых задач) неделя пометила бы 217 строк из 289 — три четверти, то
 * есть метка перестала бы что-либо выделять. Две недели дают 138, три — 78.
 *
 * Число временное по своей природе: компании сейчас месяц, и когда она
 * проживёт год, пометится снова всё. Тогда порог придётся считать от самих
 * данных — «заметно старше остальных», а не «старше N дней». Пока такого
 * запаса истории нет, лишняя механика была бы догадкой.
 */
const AGE_LABEL_DAYS = 21

function TaskAge({ createdAt }: { createdAt: string }) {
  const { t, i18n } = useTranslation()
  const days = Math.floor((Date.now() - new Date(createdAt).getTime()) / 86400000)
  if (days < AGE_LABEL_DAYS) return null
  return (
    <div
      // Точная дата — в подсказке: метка отвечает «давно», а когда именно,
      // спрашивают редко и уже прицельно.
      title={t('tasks.createdOn', {
        date: new Date(createdAt).toLocaleDateString(i18n.language, { day: 'numeric', month: 'long', year: 'numeric' }),
      })}
      className="text-[10px] text-muted-foreground/60"
    >
      {t('tasks.ageDays', { count: days })}
    </div>
  )
}

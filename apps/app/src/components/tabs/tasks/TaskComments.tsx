...
/**
 * Разметка упоминания → простой текст «@Имя».
 *
 * Только для ЦИТАТЫ над ответом: она в одну строку и без разметки, живому
 * узлу там не место. В самом комментарии тело отдаётся редактору
 * нетронутым — он подсветит упоминание сам.
 */
            ...
            <li key={c.id} className="rounded-lg border bg-card px-3 py-1.5">
              ...
              {c.deleted ? (
                ...
                <div className="msg-md break-words text-sm">
                  ...
                  {/* Тело отдаём КАК ЕСТЬ: редактор сам превращает
                      @[Имя](id) в подсвеченный узел. Раньше здесь стоял
                      renderMentions — он заранее вырезал id и оставлял голый
                      текст «@Имя», поэтому в просмотре имя не подсвечивалось,
                      а стоило нажать «редактировать» — подсветка появлялась:
                      туда тело уходило нетронутым. */}

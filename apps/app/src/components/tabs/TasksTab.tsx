  ...
  return (
    ...
    >
      ...
      {/* Номер задачи стоит ПЕРЕД названием — и в иврите тоже.
          ...
          Поэтому строка — flex. Порядок разметки и есть порядок чтения: в RTL
          первый элемент оказывается справа, то есть номер идёт ПЕРЕД
          названием — так его и ищут глазами. row-reverse здесь стоял и
          отправлял номер в хвост строки, за название.

          Номеру остаётся dir="ltr", чтобы «TASK-9» не читалось как «9-TASK»,
          а min-w-0 на названии — чтобы обрезалось многоточием именно оно, а
          не номер. */}
      <span
        className={cn(

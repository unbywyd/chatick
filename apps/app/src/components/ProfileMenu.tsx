          ...
          <DropdownMenuItem
            onSelect={async () => {
              try {
                 ...
                 * Отметки в базе мало.
                 *
                 * Показывать ли тур, решают ещё два флага в памяти экрана
                 * проекта: «закрыл щелчком мимо» и «приветствие уже было».
                 * Сбросить их отсюда напрямую нельзя — меню живёт в сайдбаре,
                 * а экран проекта ему не родитель.
                 *
                 * Ждать, что экран сам заметит смену отметки, тоже не выйдет:
                 * у того, кто закрыл тур в этом же сеансе, она уже пустая, и
                 * менять серверу нечего — экран не увидит никакого перехода.
                 *
                 * Поэтому прямой сигнал: «тур запросили заново».
                 */
                window.dispatchEvent(new CustomEvent('chatick:tour-replay'))
              } catch {

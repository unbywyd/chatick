  ...
  return (
    <div className="space-y-4">
      {/* Панель липкая: со списком в двадцать человек период и итог уезжают
          ...
          top считается, а не задан числом. Было -top-8: панель парковалась на
          32px ВЫШЕ края окна и своим непрозрачным фоном наезжала на идущий
          таймер сверху — от строки таймера оставалась видна одна середина.
          Теперь отсчёт от вкладок ПЛЮС идущий таймер (обе высоты публикует
          useStickyHeight), минус собственный pt-8: вычитаем ровно тот отступ,
          который панель сама себе добавила, иначе под вкладками осталась бы

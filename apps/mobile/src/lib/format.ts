...
/**
 * Изоляция значения, которое обязано читаться слева направо, ВНУТРИ строки на
 * другом языке: роль «admin», число, версия, почта, телефон.
 *
 * textAlign здесь не помогает — он выравнивает блок, а не переставляет знаки
 * внутри строки. Без изоляции «admin · 4» посреди ивритской фразы читается в
 * обратном порядке, а у телефона «+972…» плюс уезжает в конец. Это порча
 * данных, а не косметика.
 *
 * Изолируем ЗНАЧЕНИЕ в точке подстановки: обернуть всю фразу целиком —
 * бесполезно, фрагменту внутри это ничего не даёт.
 */
const LRI = '⁦'
const PDI = '⁩'

export const ltrValue = (v: string | number): string => `${LRI}${v}${PDI}`
